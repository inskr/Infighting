...
| M09 — Touch targets | `/`, `/search.html`, `/tags.html`, `/archive.html`, `/post.html?id=does-not-exist`, `/posts/stm32-baremetal-scheduler.html`; real touch device preferred, otherwise 320 CSS px touch emulation | Measure and operate brand/navigation links, theme button, pagination, search input/buttons, tags, like buttons, back link, TOC, related/previous/next article links, and external footer links. Every named project control must have an activation area at least **44 × 44 CSS px**; any undersized named control is a Fail. Only links embedded inline within article prose use the WCAG inline-link target-size exception. Adjacent targets must be selectable without activating the wrong item. Record dimensions and screenshots. | [ ] Pass [ ] Fail [ ] Not verified |  |
